...
class ThyroidDataset(BaseDataset):
    ...
    def __init__(self, c):
        ...
        self.tmp_file_names = ['thyroid.mat']
    ...
    def load(self):
        ...
        self.norm_samples = self.data_table [self.target == 0]  # 3679 norm
        self.anom_samples = self.data_table [self.target == 1]  # 93 anom
        ...
        self.ratio = 100.0 * (0.5*len(self.norm_samples)) / ((0.5*len(self.norm_samples)) + len(self.anom_samples))
