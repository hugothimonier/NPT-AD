...
def apply_mask(
    ...
    # Stochastic Bert Style Masking
        ...
        # mask_entries contains the indexes for each non zero value in mask_candidates
...
def gen_mask_matrices(c, target_col, D):
    ...
    ## since we ought to contrusct a matrix
    ## X = [val_num_at_time, num_train_inference]^T
    ## only the val_num_at_time rows will be masked
    ...
    if not c.exp_deterministic_masks:
        for recon in range(c.exp_num_reconstruction):
                ...
                # This gives the total number of masks sampled in our approximative
                # sampling scheme as.
                # maske sure all masks are different
                while ( (num_masks==0) or (num_masks<=recon) ):
                    ...
                    mask = torch.sparse.FloatTensor(
                        mask_indices.T,
                        torch.ones(len(mask_indices), dtype=torch.int),
                        mask_candidates.size(),
                        ).to_dense().type(torch.bool)

                    if not any([(mask == c_).all() for c_ in existing_masks]):
...
def apply_mask_val_ad_for_dataset(c, 
    ...
    for col, data_arr in enumerate(data_arrs):
        ...
        # If there are no masks in this column, continue.
        if mask_col == 0:
            masked_tensors.append(data_arr.type(data_dtype))
            continue
            
        masked_arr = copy.deepcopy(data_arr)
            
        masked_arr[train_idx:,0] = 0
        masked_arr[train_idx:,1] = 1
