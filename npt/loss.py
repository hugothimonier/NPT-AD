...
class Loss:
    ...
    * Loss.finalize_epoch_losses() to obtain the loss_dict for the current
    ...
    def compute_loss(
            ...
            augmentation_mask_matrix, dataset_mode, eval_model, val_batchsize:int=1):
        ...
        if 'row_index' in list(data_dict.keys()):
            self.row_index = data_dict['row_index']
        else:
            data_dict['row_index'] = None
        
        if 'old_to_new' in list(data_dict.keys()):
            self.old_to_new = data_dict['old_to_new']
        else:
            self.old_to_new = None
            
        if 'new_to_old' in list(data_dict.keys()):
            self.new_to_old = data_dict['new_to_old']
        else:
            self.new_to_old = None
            
        if 'old_indice_to_target' in list(data_dict.keys()):
            self.old_indice_to_target_val = data_dict['old_indice_to_target']
        else:
            self.old_indice_to_target_val = None
            ...
            for mode, mode_loss_indices in loss_indices.items():
                ...
                if is_cat:
                    ...
                    if eval_model:
                        ...
                        self.val_loss_logg['num_val_pred'] += num_preds
                        self.val_loss_logg['loss_val_epoch'] += loss_dict[mode]['cat_loss']
                else:
                    ...
                    if eval_model:
                        self.val_loss_logg['num_val_pred'] += num_preds
                        self.val_loss_logg['loss_val_epoch'] += loss_dict[mode]['num_loss']
    ...
    def finalize_epoch_losses(self, eval_model, final_rec:bool=False):
        """Normalise epoch losses and reset stored values."""
        std_dict = self.finalize_losses(self.epoch_loss, eval_model)

        if final_rec:
            return std_dict, self.loss_val
        else:
            return std_dict, None
        ...
        if is_cat:
            ...
            if val_batchsize:
                for i in range(val_batchsize):
                    old_idx = self.new_to_old['val'][self.row_index + i]
                    if old_idx in self.current_loss_val.keys():
    ...
    def normalize_and_finalize_loss(self):
        
        val_tensors = torch.stack(list(self.current_loss_val.values()))

        if torch.count_nonzero(val_tensors) == 0:
            return

        mean, std = (torch.mean(val_tensors), 
                    torch.std(val_tensors))
        
        for idx, value in self.current_loss_val.items():
            if idx in self.loss_val.keys():
                self.normalized_loss_val[idx].append( (value - mean ) / std) 
            else:
                self.normalized_loss_val[idx] = [( value - mean ) / std]
                
        for idx, value in self.current_loss_val.items():
            if idx in self.loss_val.keys():
                self.loss_val[idx].append(value) 
            else:
                self.loss_val[idx] = [value] 
