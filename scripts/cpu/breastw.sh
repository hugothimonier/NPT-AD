...
rm -rf ./data/breastw/breastw/*
export Q_DATE=$(date "+%d_%m_%y_%H_%M") 
mkdir -p ./logs/breastw
mkdir -p ./results/breastw

python -u run.py \
    --data_path ./data/breastw/ \
    --data_set breastw \
    --res_dir job_$Q_DATE\
    --data_loader_nprocs 0 \
    --data_set_on_cuda False \
    --exp_name breastw_params \
    --np_seed 1 \
    --torch_seed 1 \
    ...
    --model_rff_depth 1 \
    --ad True \
    --data_force_reload True > ./logs/breastw/$Q_DATE.log