...
rm -rf ./data/mullcross/mullcross/*
export Q_DATE=$(date "+%d_%m_%y_%H_%M") 
mkdir -p ./logs/mullcross
mkdir -p ./results/mullcross

python -u run.py \
    --n_runs 20 \
    --data_path ./data/mulcross/ \
    --data_set mulcross \
    --res_dir job_$Q_DATE\
    --data_loader_nprocs 0 \
    --data_set_on_cuda False \
    --exp_name mullcross_params \
    --np_seed 1 \
    --torch_seed 1 \
    --original_torch_seed 1 \
    --exp_batch_size 4096 \
    --exp_val_batchsize 100 \
    --exp_num_train_inference 3096 \
    --exp_num_total_steps 10000 \
    ...
    --model_dim_hidden 16 \
    --model_augmentation_bert_mask_prob "dict(train=0.15, val=0.5, test=0.15)" \
    ...
    --model_rff_depth 1 \
    --ad True \
    --data_force_reload True > ./logs/mullcross/$Q_DATE.log