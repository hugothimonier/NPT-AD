...
rm -rf ./data/kddrev/kddrev/*
export Q_DATE=$(date "+%d_%m_%y_%H_%M") 
mkdir -p ./logs/kddrev
mkdir -p ./results/kddrev

python -u run.py \
    --n_runs 10 \
    --data_path ./data/kddrev/ \
    --data_set kddrev \
    --res_dir job_$Q_DATE\
    --data_loader_nprocs 0 \
    --data_set_on_cuda False \
    --exp_name kddrev_params \
    --np_seed 1 \
    --torch_seed 1 \
    --original_torch_seed 1 \
    --exp_batch_size 4096 \
    --exp_val_batchsize 1048 \
    --exp_num_train_inference 4096 \
    --exp_num_total_steps 1000000.0 \
    ...
    --model_dim_hidden 16 \
    --model_augmentation_bert_mask_prob "dict(train=0.15, val=0.15, test=0.1)" \
    ...
    --model_rff_depth 1 \
    --ad True \
    --data_force_reload True > ./logs/kddrev/$Q_DATE.log  