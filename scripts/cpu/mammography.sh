...
rm -rf ./data/mammography/mammography/*
export Q_DATE=$(date "+%d_%m_%y_%H_%M") 
mkdir -p ./logs/mammography
mkdir -p ./results/mammography

python -u run.py 
    --n_runs 20 \
    --data_path ./data/mammography/\
    --data_set mammography\
    --res_dir job_$Q_DATE\
    --data_loader_nprocs 0 \
    --data_set_on_cuda False\
    --exp_name mammo_params \
    --np_seed 1 \
    --torch_seed 1 \
    ...
    --exp_num_total_steps 200 \
    --exp_eval_every_n 200 \
    ...
    --model_dim_hidden 16 \
    --model_augmentation_bert_mask_prob "dict(train=0.25, val=0.5, test=0.15)" \
    ...
    --model_rff_depth 1 \
    --ad True \
    --data_force_reload True > ./logs/mammography/$Q_DATE.log