...
rm -rf ./data/speech/speech/*
export Q_DATE=$(date "+%d_%m_%y_%H_%M") 
mkdir -p ./logs/speech
mkdir -p ./results/speech
...
python -u run.py \
    ...
    --exp_name speech_params \
    --np_seed 1 \
    --torch_seed 1 \
    --original_torch_seed 1 \
    --exp_batch_size 512 \
    --exp_val_batchsize 100 \
    --exp_num_train_inference 412 \
    --exp_num_total_steps 8500 \
    ...
    --model_dim_hidden 8 \
    --model_augmentation_bert_mask_prob "dict(train=0.15, val=0.25, test=0.15)" \
    ...
    --model_rff_depth 1 \
    --ad True \
    --data_force_reload True > ./logs/speech/$Q_DATE.log