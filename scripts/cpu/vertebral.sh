...
rm -rf ./data/vertebral/vertebral/*
export Q_DATE=$(date "+%d_%m_%y_%H_%M") 
mkdir -p ./logs/vertebral
mkdir -p ./results/vertebral

python -u run.py \
    --n_runs 20 \
    --data_path ./data/vertebral/ \
    --data_set vertebral \
    --res_dir job_$Q_DATE \
    --data_loader_nprocs 0 \
    --data_set_on_cuda False \
    --exp_name vertebral_params \
    --np_seed 1 \
    --torch_seed 1 \
    --original_torch_seed 7 \
    --exp_batch_size -1 \
    --exp_val_batchsize 10 \
    --exp_num_train_inference -1 \
    --exp_num_total_steps 2000 \
    --exp_eval_every_n 2000 \
    ...
    --model_rff_depth 1 \
    --ad True \
    --data_force_reload True > ./logs/vertebral/$Q_DATE.log