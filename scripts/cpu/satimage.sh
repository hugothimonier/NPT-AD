...
rm -rf ./data/satimage/satimage/*
export Q_DATE=$(date "+%d_%m_%y_%H_%M") 
mkdir -p ./logs/satimage
mkdir -p ./results/satimage

python -u run.py \
    --n_runs 20 \
    --data_path ./data/satimage/ \
    --data_set satimage \
    --res_dir job_$Q_DATE \
    --data_loader_nprocs 0 \
    --data_set_on_cuda False \
    --exp_name satimage_params \
    --np_seed 1 \
    --torch_seed 1 \
    ...
    --model_dim_hidden 16 \
    --model_augmentation_bert_mask_prob "dict(train=0.2, val=0.5, test=0.15)" \
    ...
    --model_rff_depth 1 \
    --ad True \
    --data_force_reload True > ./logs/satimage/$Q_DATE.log