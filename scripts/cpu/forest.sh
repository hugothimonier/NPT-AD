...
rm -rf ./data/forest-cover/forest-cover/*
export Q_DATE=$(date "+%d_%m_%y_%H_%M") 
mkdir -p ./logs/forest-cover
mkdir -p ./results/forest-cover

python -u run.py \
    --n_runs 20 \
    --data_path ./data/forest_cover/ \
    --data_set forest-coverAD \
    --res_dir job_$Q_DATE \
    --data_loader_nprocs 0 \
    --data_set_on_cuda False \
    --exp_name forestcover_params \
    --np_seed 1 \
    --torch_seed 1 \
    --original_torch_seed 1 \
    --exp_batch_size 4096 \
    --exp_val_batchsize 200 \
    --exp_num_train_inference 3896 \
    --exp_num_total_steps 100000 \
    ...
    --model_rff_depth 1 \
    --ad True \
    --data_force_reload True > ./logs/forest/$Q_DATE.log